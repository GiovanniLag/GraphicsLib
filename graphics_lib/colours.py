...
class Palette:
    ...
    def save_palette(self, filename):
        ...
        """
        import json
        # Convert Colour objects to serializable format
        palette_data = {
            'name': self.name,
            'colours': {
                key: str(colour) for key, colour in self.colours.items()
            }
        }
    ...
    def load_palette(cls, filename):
        ...
        # Convert hex strings back to Colour objects
        colours = {
            key: Colour(hex_color)
            for key, hex_color in palette_data['colours'].items()
        }
        return cls(palette_data['name'], colours)
